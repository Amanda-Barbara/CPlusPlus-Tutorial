...
#### [右值引用](./rvalue_reference_test.cpp)

右值引用就是必须绑定到右值（一个临时对象、将要销毁的对象）的引用，具名的右值引用是一个左值，地址与绑定的对象一致。
...
## 参考链接
* 1 [右值引用](https://zh.wikipedia.org/wiki/%E5%8F%B3%E5%80%BC%E5%BC%95%E7%94%A8)
