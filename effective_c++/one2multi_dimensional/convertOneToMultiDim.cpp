...
int main()
{
   ...
   for(int i=0;i<row;i++)
   {
       for(int j=0;j<col;j++)
       {
          printf("output0[%d,%d]: %f; output1[0,%d]: %f.\n", i, j, output0[i][j], j, output1[0][j]);
       }
   }
   ...
   printf("buffer=%lx, output0=%lx, output1=%lx\n", buffer, output0, output1);  //以十六进制形式输出（字母小写）
   printf("buffer=%ld, output0=%ld, output1=%ld\n", buffer, output0, output1);  //以十进制形式输出
